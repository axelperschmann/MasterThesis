...
\begin{algorithm}[H] 
 \caption{Optimal\_strategy, as described in \Cite{Nevmyvaka:2006}}
     ...
     \KwIn{Transition set $\widetilde{D} = \{(x_t, a_t, s_t, r_t) $ $|$ $ t=1..n \}$}
...
\begin{algorithm}[H] 
 \caption{Forward sampling approach}
     \SetAlgoLined
     \footnotesize
...
\While{not end of data}{
init OTS with $V=100\%$ and current orderbook\_window
    \For{epoch=0 to E}{
        Reset OTS to random start point\;
        \Repeat{until $V=0\%$}{
        Transform (orderbook) $\rightarrow o_1..o_R$\;
        Enquire $\epsilon$-greedy action\;
        \If{action led to an end state previously}{choose other action}
        apply action
